...
BTNode *factor(void) {
    ...
    if (match(INT)) {
        ...
        retp = makeNode(LOGICAL, getLexeme());
        retp->left = makeNode(INT, "0");
        advance();
        if (match(INT)) {
            retp->right = makeNode(INT, getLexeme());
...
void statement(void) {
    ...
    if (match(ENDFILE)) {
        printf("MOV r0 [0]\n"); // move x back to register0;
        printf("MOV r1 [4]\n"); // move y back to register0;
        printf("MOV r2 [8]\n"); // move z back to register0;
        printf("EXIT 0\n");
        exit(0);
    } else if (match(END)) {
        //printf(">> ");
